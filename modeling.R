...
train_all <- prepare_text(sample_train$all_text)
valid_all <- prepare_text(sample_valid$all_text)
...
values_train <- as.integer(sample_train$unhygienic[doc_train_totals > 0])
...
values_valid <- as.integer(sample_valid$unhygienic[doc_valid_totals > 0])
...
t1 <- Sys.time()
slda_trained <- slda.em(documents = docs_train, vocab =  vocab, K = k, num.e.iterations = 20, num.m.iterations = 10, variance = 0.25, 
...
top_words<- top.topic.words(slda_trained$topics, num.words = 20)
topic_coeff <- as.data.frame(slda_trained$coef)

topics <- as.data.frame(slda_trained$topics)
