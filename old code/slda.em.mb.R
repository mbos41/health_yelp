          ...
          MaxNWts = 3000, augment) 
{
  ...
  estimate.params <- function(document.sums, num.topics, logistic, 
    ...
    if (logistic) {
      ...
      model.fit <- nnet::multinom(annotations ~ z.bar. + augment + 
                                    0, family = multinom(), MaxNWts = MaxNWts, trace = trace)
    }
    else if (regularise) {
      model.fit <- penalized::penalized(annotations ~ 
                                          z.bar., unpenalized = ~0, lambda2 = 1/lambda^2, 
                                        trace = FALSE)
    }
    else {
  ...
  for (ii in seq(length.out = num.m.iterations - 1)) {
    result <- .slda.collapsed.gibbs.sampler(documents, K, 
                                            ...
                                            params, variance[ii + 1], logistic, method[ii + 
                                                                                         1], lambda = lambda, initial = list(assignments = result$assignments), 
                                            trace = trace)
    fit <- estimate.params(result$document_sums, K, logistic, 
                           MaxNWts)
    params <- fit$coefs
